...
parser.add_argument('--split_length', type=int, default=25, help='split dim')
...
if __name__ == '__main__':
    ...
    for i in range(dim//split_length+1):
        if(dim>(i+1)*split_length):
            train_sub_data = train_data[:,i*split_length:(i+1)*split_length]
        else:
            train_sub_data = train_data[:,i*split_length:dim]
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_data_loader):
            for x, y in zip(batch_x,batch_y):
                ...
                for i in range(dim//split_length+1):
                    ...
                    if (dim > (i + 1) * split_length):
                        x_diff_sub = x_diff[:, i * split_length:(i + 1) * split_length]
                    else:
                        x_diff_sub = x_diff[:, i * split_length:dim]
