...
from layers.NormalizingFlow import NormalizingFlow
from torch.distributions import Normal
# Cell
class PatchNorm_backbone(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
                 ...
                 verbose:bool=False,normflow=True,nflow_dim=16,nflow_layers=2,nflow_weight=0.1,**kwargs):
        ...
        if normflow:
            # norm-flow
            self.NormalizingFlow =NormalizingFlow(c_in,target_window,dropout,nflow_dim=nflow_dim)
    ...
    def forward(self, z):                                                                   # z: [bs x n_vars x seq_len]
        ...
        if self.normflow:
            ####################### norm - flow ####
            sample = self.NormalizingFlow(z).permute(0,2,1)
        ...
        # model
        z = self.backbone(z)                                                                # z: [bs x n_vars x d_model x patch_num]
        z = self.head(z)                                                                    # z: [bs x n_vars x target_window] 
        if self.normflow:   z = sample*0.1 +z*0.9
        # denorm
        if self.revin: 
            z = z.permute(0,2,1)
...
# https://github.com/PaddlePaddle/PaddleSpatial/blob/main/research/Conformer/models/model.py
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
        ...
        self.encoder = TSTEncoder(q_len, d_model, n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm, attn_dropout=attn_dropout, dropout=dropout,
                                   pre_norm=pre_norm, activation=act, res_attention=res_attention, n_layers=n_layers, store_attn=store_attn,attn_mask=attn_mask)
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x n_vars x patch_len x patch_num]
        ...
        x = self.W_P(x)                                                          # x: [bs x n_vars x patch_num x d_model]

        u = torch.reshape(x, (x.shape[0]*x.shape[1],x.shape[2],x.shape[3]))      # u: [bs * n_vars x patch_num x d_model]
        u = self.dropout(u + self.W_pos)                                         # u: [bs * n_vars x patch_num x d_model]

        # Encoder
        z = self.encoder(u)                                                      # z: [bs * n_vars x patch_num x d_model]
...
class TSTEncoder(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=None, 
                        ...
                        res_attention=False, n_layers=1, pre_norm=False, store_attn=False,attn_mask=None):
        super().__init__()
