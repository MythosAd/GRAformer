...
from models.VAR import VARModelTorchGPU
import argparse
...
if __name__ == '__main__':
    ...
    lag_order = 49 # args.seq_len-1
    # 步骤 4: 创建和训练 VAR 模型 , 因为是差分后的数据,将回望窗口-1符合差分回复。
    model = VARModelTorchGPU(diff_train_data,lag_order)
    model.fit()
    ...
    for seq in (96,192,336,720):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_data_loader):
            for x, y in zip(batch_x,batch_y):
                ...
                forecast = model.predict(x_diff[-lag_order:], steps=seq)
                # 步骤 6: 将差分预测转换回原始尺度
                # 累计求和（逆差分）
                forecast = np.cumsum(np.vstack((x[-1], forecast)), axis=0)[1:]
        ...
        print(f'MSE: {mse}')
        print(f'MAE: {mae}')
        print('mse:{}, mae:{},lag_order:{}'.format(mse, mae,lag_order))
        f = open("result.txt", 'a')
        f.write("VAR_" + str(args.data_path) + "_" + str(args.seq_len) + "_" + str(seq) + "\n")
        f.write('mse:{}, mae:{},lag_order:{}'.format(mse, mae,lag_order))
        f.write('\n')
        f.write('\n')
        f.close()
