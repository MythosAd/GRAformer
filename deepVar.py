...
if __name__ == '__main__':
    ...
    train_dataset = ListDataset([{
        ...
        "start":'1980-07-01 00:00:00',
        "target":  train_data_set.data_x.T
    }],one_dim_target = False, freq="M")

    vali_dataset =  ListDataset([{
        #"start": df[df.columns[0]][0],
        "start":'2000-07-01 00:00:00',
        "target":  val_data_set.data_x.T
    }],one_dim_target = False, freq="M")

    test_dataset = ListDataset([{
        #"start": df[df.columns[0]][0],
        "start":'2020-07-01 00:00:00',
        "target": test_data_set.data_x.T
    }],one_dim_target = False, freq="M")
    ...
    model = DeepVAREstimator(
        ...
        context_length=args.seq_len,prediction_length=args.pred_len, freq="M",
        trainer =Trainer()
    ).train(training_data =train_dataset,validation_data=vali_dataset)
    ...
    forecast_it, ts_it = make_evaluation_predictions(
        ...
        predictor=model,  # 训练好的模型
        num_samples=2000,  # 用于生成预测的样本数
    )
    ...
    # evaluator = Evaluator(quantiles=[0.5])
    # agg_metrics, item_metrics = evaluator(test_dataset, forecasts, num_series=len(test_dataset))
    ...
    f = open("result.txt", 'a')
    f.write("deepVAR_" +str(args.data_path)+"_" +str(args.seq_len)+"_" +str(args.pred_len)+"\n")
