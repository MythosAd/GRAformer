...
class ResiDualEncoderLayer(TransformerEncoderLayerBase):
    ...
    def forward(self, data: Tuple[Tensor, Tensor], encoder_padding_mask: Optional[Tensor],
        ...
        x = self.self_attn_layer_norm(x + xr) # first
        ...
        x = self.dropout_module(x)
        res = res + x * self.cfg.enc_res_input_norm_scale
        x = self.final_layer_norm(x + xr) # second
