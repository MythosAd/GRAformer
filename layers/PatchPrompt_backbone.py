...
from layers.PatchTST_layers import positional_encoding_3D
...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
                 ...
                 subtract_last=False,
                 verbose: bool = False, pe_3d=False, **kwargs):
        ...
        self.backbone = TSTiEncoder(c_in, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,
                                    ...
                                    pe=pe, learn_pe=learn_pe, verbose=verbose, pe_3d=pe_3d, **kwargs)

        # Head
        self.head_nf = d_model * (patch_num + 1)
        self.n_vars = c_in
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
                 ...
                 pe='zeros', learn_pe=True, verbose=False, pe_3d=False, **kwargs):
        ...
        self.prompt = prompt_token(pe, learn_pe, c_in, d_model, token_num=1)
    ...
    def forward(self, x) -> Tensor:  # x: [bs x n_vars x patch_len x patch_num]
        ...
        if self.pe_3d:
            ...
            bs = x.shape[0]
            prompts = self.prompt.unsqueeze(0).repeat(bs, 1, 1, 1)  # n_vars x 1 x d_model
            prompts = torch.reshape(prompts, (prompts.shape[0] * prompts.shape[1], prompts.shape[2], prompts.shape[3]))

            u = torch.cat((prompts, u), 1)

        # Encoder
        if self.attn_mask:
            attn_mask = nn.Transformer.generate_square_subsequent_mask(self.patch_num, device=x.device)
            z = self.encoder(u, attn_mask=attn_mask)  # z: [bs * n_vars x patch_num x d_model]
        else:
            z = self.encoder(u)
