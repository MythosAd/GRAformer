...
class GPT4TSPrompt(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        self.prompt = self.prompt_token('zeros', True, 3, configs.d_model, token_num=configs.prompt_token).to(
            device=device)
        ...
        # self.out_layer = nn.Linear(configs.d_model * (self.patch_num + configs.prompt_token), configs.pred_len).to(
        #     device=device)
